...
- [X] Write `src/config.py` — loads `GEMINI_API_KEY` and `OBSIDIAN_VAULT_PATH` from env, exposes typed constants, raises clear errors if required vars are missing
- [X] Write `src/main.py` — parses `--mood` CLI argument, initialises config, prints a placeholder response to stdout
- [X] Remove root `main.py` generated by `uv init`
- [X] Update `pyproject.toml` — add description and `[project.scripts]` entry pointing to `src.main:main`
